...
void main() {
  group('PBKDF2-HMAC-SHA256 (RFC 7914 §11)', () {
    test('passwd "passwd" / salt "salt" / 1 iter / 64B', () {
      final dk = pbkdf2Worker([utf8.encode('passwd'), utf8.encode('salt'), 1, 64]);
      const expected =
    ...
    test('PBKDF2 deterministic — same input → same output', () {
      final a = pbkdf2Worker([utf8.encode('mypass'), utf8.encode('s'), 1000, 32]);
      final b = pbkdf2Worker([utf8.encode('mypass'), utf8.encode('s'), 1000, 32]);
      expect(_hex(a), _hex(b));
    });

    test('PBKDF2 changes with salt', () {
      final a = pbkdf2Worker([utf8.encode('pass'), utf8.encode('salt1'), 100, 32]);
      final b = pbkdf2Worker([utf8.encode('pass'), utf8.encode('salt2'), 100, 32]);
      expect(_hex(a), isNot(equals(_hex(b))));
    });

    test('PBKDF2 changes with iterations', () {
      final a = pbkdf2Worker([utf8.encode('pass'), utf8.encode('salt'), 100, 32]);
      final b = pbkdf2Worker([utf8.encode('pass'), utf8.encode('salt'), 200, 32]);
      expect(_hex(a), isNot(equals(_hex(b))));
    });
  });
  ...
  group('AES-256-CBC round-trip', () {
    ...
    test('different IVs produce different ciphertext for same key+plaintext', () {
      final key = enc.Key(Uint8List.fromList(List<int>.generate(32, (i) => i)));
    ...
    test('wrong key fails to decrypt cleanly', () {
      final keyOk = enc.Key(Uint8List.fromList(List<int>.generate(32, (i) => i)));
      final keyBad = enc.Key(Uint8List.fromList(List<int>.generate(32, (i) => i + 1)));
      final iv = enc.IV(Uint8List.fromList(List<int>.generate(16, (i) => i)));
      final ct = enc.Encrypter(enc.AES(keyOk, mode: enc.AESMode.cbc))
          .encrypt('secret', iv: iv);
      expect(
        () => enc.Encrypter(enc.AES(keyBad, mode: enc.AESMode.cbc))
            .decrypt(ct, iv: iv),
        throwsA(anything),
      );
    });
  ...
  group('Encrypt-then-MAC pattern (used by vault_service)', () {
    test('PBKDF2 → AES-CBC encrypt → HMAC verify → AES-CBC decrypt', () {
      ...
      final ct = encrypter.encrypt(plain, iv: iv);
      final mac = Hmac(sha256, macKey).convert([...iv.bytes, ...ct.bytes]).bytes;

      // Vérification HMAC AVANT déchiffrement.
      final computed = Hmac(sha256, macKey).convert([...iv.bytes, ...ct.bytes]).bytes;
    ...
    test('tampered ciphertext is detected by HMAC', () {
      ...
      tampered[0] ^= 0xff;
      final computed =
          Hmac(sha256, macKey).convert([...iv.bytes, ...tampered]).bytes;
